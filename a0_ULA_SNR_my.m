...
for xaxis = 1:length(xAxes)
    ...
    for nsim=1:Nsim
        ...
        disp(' ');disp(['SNR',num2str(xAxes(xaxis)),'#Sim : ',num2str(nsim)])
        ...
        SNR = xAxes(xaxis);        
        % total number of snapshots;% Nsnapshot = 10;        
        % range of angle space
        thetalim = [-90 90];
        % Angular search grid
        theta_separation = .25;        theta = (thetalim(1):theta_separation:thetalim(2))';        Ntheta = length(theta);        
        % Design/steering matrix (Sensing matrix)
        sin_theta = sind(theta);        sensingMatrix = exp(-1i*2*pi/lambda*xq*sin_theta.')/sqrt(Nsensor);
        % Generate received signal        
        anglesTrue = [-65; -2; 3];        anglesTrue = anglesTrue + rand(size(anglesTrue)) - 0.5;
        ...
        vanTreeD = (-1i*2*pi/lambda*xq*cosd(anglesTracks(:,snapshot)).') .* exp( -1i*2*pi/lambda*xq*sinAnglesTracks(:,snapshot).' ); % D Eq.(8.100)D Eq.(8.97) of [1]        
        Xs = cX;
        ...
        H = vanTreeD' *(eye(Nsensor) - vanTreeV/(vanTreeV'*vanTreeV)*vanTreeV')*vanTreeD;        
        % det. CRB Eq.(8.110)
        ...
        %% AP-ULA        
        MAX_IT = 1000;        NeigAP = Nsource;        tol    = 1e-3;
